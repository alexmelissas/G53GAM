...
public class BattleResultPopup : MonoBehaviour {
    ...
    private void Update()
    {
        if(BattleScreen.updatePlayer!=-1 && didSetup==false)
        {
            Setup();
            didSetup = true;
        }
    ...
    private void Setup()
    {
        before = PersistentObjects.singleton.player_before_battle;
        after = PersistentObjects.singleton.player;
        
        addedExp = 0;

        if(BattleScreen.updatePlayer==1)
        {
